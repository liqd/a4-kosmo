...
@pytest.mark.django_db
def test_fields(apiclient,
                ai_classification_factory,
                user_classification_factory,
                comment_factory,
                idea, moderator_comment_statement_factory):
    comment_1 = comment_factory(content_object=idea,
                                is_moderator_marked=True)
    classification_1 = user_classification_factory(
        comment=comment_1,
        is_pending=False
    )
    ...
    classification_3 = ai_classification_factory(
        comment=comment_3,
        is_pending=True
    )
    statement = moderator_comment_statement_factory(comment=comment_3)
    ...
    assert comment_1_data['last_edit'] == \
           dates.get_date_display(comment_1.created)
    assert comment_1_data['moderator_statement'] is None
    assert comment_1_data['num_active_notifications'] == 1
    assert comment_1_data['pk'] == comment_1.pk
    assert comment_1_data['statement_api_url'] == \
        reverse('moderatorstatement-list',
                kwargs={'comment_pk': comment_1.pk})
    assert comment_1_data['time_of_last_notification'] == \
        dates.get_date_display(classification_1.created)
    assert comment_1_data['user_image'] == \
        comment_1.creator.avatar_fallback
    assert comment_1_data['user_name'] == comment_1.creator.username
    assert comment_1_data['user_profile_url'] == \
    ...
    assert comment_2_data['statement_api_url'] == \
        reverse('moderatorstatement-list',
                kwargs={'comment_pk': comment_2.pk})
    assert comment_2_data['time_of_last_notification'] == \
        dates.get_date_display(classification_2.created)
    assert comment_2_data['user_image'] == \
        comment_2.creator.avatar_fallback
    assert comment_2_data['user_name'] == comment_2.creator.username
    assert comment_2_data['user_profile_url'] == \
    ...
    assert comment_3_data['last_edit'] == \
           dates.get_date_display(comment_3.created)
    assert comment_3_data['moderator_statement']['statement'] == \
        statement.statement
    assert comment_3_data['num_active_notifications'] == 2
    assert comment_3_data['pk'] == comment_3.pk
    assert comment_3_data['statement_api_url'] == \
        reverse('moderatorstatement-list',
                kwargs={'comment_pk': comment_3.pk})
    assert comment_3_data['time_of_last_notification'] == \
        dates.get_date_display(classification_3.created)
